...
const inputLocal = document.querySelector(".Local-Input");
const inputGuest = document.querySelector(".guest-Input");

const opcoesLocal = document.querySelector('.opcoes-lugares');
const opcoesGuest = document.querySelector('.opcoes-guest');
const opcao = document.querySelectorAll(".opcao-local");
...
//Opções de Cidades e Filtro de pessoas
inputLocal.addEventListener('click', () => {
  opcoesLocal.classList.toggle("active")
  ...
  for (let i = 0; i < json.length; i++) {
    if (values === json[i].city.toLowerCase() && totalGuest() <= json[i].maxGuests) {
      ...
      <div class="container-lugares">
        <div class="info-lugares">
          <div class="img-lugares">
            <img  class="img-photo"src=${imgQuarto} alt="">
          </div>
          <div class="detalhes">
          <div class="detalhes-lugares">
            ${superHostString}
            <span>
            ${typeLocation}
            </span>
